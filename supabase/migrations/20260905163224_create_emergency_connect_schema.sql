/*
# Emergency Connect — Full Database Schema

1. New Tables
- `profiles` — extends auth.users with name, phone, role (citizen/vehicle), vehicle_type, vehicle_code, saved_location
- `vehicles` — emergency vehicles (ambulance, fire, police) with status, driver info, location
- `hospitals` — hospital listings with emergency availability, beds, contact info
- `fire_stations` — fire station listings with vehicle count, contact info
- `police_stations` — police station listings with unit count, contact info
- `ambulance_stations` — ambulance hub listings with ambulance count, contact info
- `emergency_requests` — citizen emergency requests with service type, emergency type, pickup location, patients, status, assigned vehicle
- `alerts` — alert notifications for citizens and vehicle users
- `emergency_contacts` — citizen emergency contacts (name, phone, relation)

2. Security
- RLS enabled on all tables.
- profiles: owner-scoped (authenticated users see/edit only their own profile).
- vehicles/hospitals/fire_stations/police_stations/ambulance_stations: public read for all (anon + authenticated), no writes from frontend.
- emergency_requests: owner-scoped (citizens see only their own requests).
- alerts: owner-scoped (users see only their own alerts).
- emergency_contacts: owner-scoped (users see only their own contacts).
- All writes use auth.uid() for ownership checks.
- user_id columns default to auth.uid() so inserts that omit user_id still pass RLS.
*/
...
BEGIN
  ...
  VALUES (NEW.id, COALESCE(NEW.raw_user_meta_data->>'name', ''), '', 'citizen')
